...
- **tabs/** — `useTabs` is the source of truth for tab list + active id. `useWorkspaceCwd` derives explorer root + inherited cwd for new tabs from active tab, now taking `projectRoot` parameter (new priority chain: active terminal tab's live cwd / last terminal cwd / open project root / default directory setting / workspace home / undefined). `basename` splits on both `/` and `\`.
...
- **workspace/** — workspace environment switching (Local + WSL distros), project root initialization on cold launch via `initialProjectRoot.ts` (authorizes persisted root and seeded boot directory), and follow-the-project behavior: when the active project root changes, idle terminals (OSC 133 shell integration) receive `cd <root>` injection via `writeToSession`; trigger decision is via `shouldSyncTerminalsToRoot()` (excludes closed folder, same root re-select, cold-launch restore). Terminal idle state is read from OSC 133 `inCommand` persistence on the session's `shellState` field, not trapped in closure.
...
- **Terminal idle detection** for project-root follow: reads OSC 133 shell integration state (`inCommand` persistence on `shellState`). A non-integrated shell mid-TUI will read as idle and receive `cd` injection; acceptable in practice, residual risk to note if expanding the follow behavior.
